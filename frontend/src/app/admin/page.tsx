...
export default function SuperAdminPage() {
  ...
  useEffect(() => {
    const cargarGyms = async () => {
      try {
        const respuesta = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/gyms`);
  ...
  const toggleModulo = async (gymId: string, modulo: keyof Gym["modulos"]) => {
    ...
    try {
      const respuesta = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/gyms/${gymId}/modules`, {
        method: "PATCH",
  ...
  const toggleGymStatus = async (gymId: string, currentStatus: boolean) => {
    ...
    try {
      const respuesta = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/gyms/${gymId}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
